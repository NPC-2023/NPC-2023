...
function scene:create( event )
	...
	--초기화
	--loadedSettings.custom1 = "image/custom/cat.png"
	--loadedSettings.custom2 = "image/custom/투명.png"
	...
	map_text.size = 40
 	...
 	local panel = {} --panel 9+i->옷 3개 panel 12+i->옷 3
 	for i = 1,3 do
    ...
    audio.play(custom)
  	...
  	--loadedSettings.custom2 = "image/custom/투명.png"
 	--loadedSettings.custom1 = "image/custom/cat.png"
  	--loadedSettings.clothesFlag = 1
	cloth_count = loadedSettings.clothesCount
    -------------
	...
	--선물 주는 코드
	--loadedSettings.clothesCount = 9 --현재 옷 개수
	cloth_count = loadedSettings.clothesCount
	print(cloth_count)

 	local r_gift = composer.getVariable("receiveClothes") --히든 퀘스트 수행 후 옷 선물을 받았을 때 
 	
 	if(r_gift == 1) then --우편함에서 선물을 받았다
 		cloth_count = cloth_count + 1
 		loadedSettings.clothesCount = cloth_count
 	end

	local closet_num = 1 --화살표 이용해서 옷장 넘기는 number 초기값: 1
 	if(cloth_count == 3) then --옷이 3개 있을 때(기본 수량) 
		closet_num = 1
	elseif(cloth_count >= 4 and cloth_count <= 6) then --옷이 4~6개 있을 때
		closet_num = 2 
	else --옷이 6개 보다 많을 때
		closet_num = 3
	end
	...
	local function right(event) --오른쪽으로 이동
		--print(right_num)
		if(right_num < closet_num) then --둘 다 1일 때는 넘기기 불가능. closet_num이 2, 3 일때 오른쪽으로 이동 가능
			...
			if(right_num == 2) then --right_num==2 -> 봄 여름 여름 
				...
				for i= 4, 6 do --4~6개 니까 옷이 총 5개면 4, 5만 보이게.
					if(i <= cloth_count) then
						clothes[i].alpha = 1
					end
				end
	...
	local function left(event) --왼쪽으로 이동
		if(left_num > 1) then --closet_num이 1일 때 이동 불가능	
			left_num = left_num - 1 
			right_num = right_num - 1
			if(left_num == 1) then --closet_num이 2일 때
				for i = 1,3 do
					panel[6+i].alpha = 1 --기본 옷 3개 panel
					clothes[i].alpha = 1 --기본 옷 3개
