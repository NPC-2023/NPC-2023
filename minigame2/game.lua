...
function scene:create( event )
	...
	---------1차시-------------
	local background = display.newImageRect("image/cafeteria.png", 1280, 720)--배경이미지 display.contentWidth, display.contentHeight
 					...
 					and event.target.y > pan.y - 300 and event.target.y < pan.y + 300) then--- 50 50
 						if (event.target == food[1] or event.target == food[2] or event.target == food[5] or event.target == food[6] or event.target == food[9]) then
 							...
 							if(score.text == '5') then
 								score.text = '성공!'
 								time.alpha = 0
								composer.removeScene("game")--다현님
								composer.setVariable("score", 5)---다현님
								composer.gotoScene( "view2" )---다현님 veiw2가 게임오버창 
 							end
 	...
 	local function counter( event )
 		...
 		if( time.text == '-1') then
 			...
 			if( score.text ~= '성공!' ) then
 				score.text = '실패!'
 				composer.removeScene("game")---다현님코드 
				composer.setVariable("score", -1)---다현님 코드 
				composer.gotoScene("view2")---다현님 코드 
