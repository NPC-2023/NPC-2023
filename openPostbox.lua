...
function scene:create( event )
   ...
   -- music = loadedEndings.bgMusic
    ...
    local p = display.newImageRect("image/postbox/우체통클릭화면.png", 750, 750)
    p.x, p.y = display.contentWidth/2, display.contentHeight/2
    sceneGroup:insert(p)
   
    local c_box = {}
    local objectGroup = display.newGroup()
    for i=1, 6 do 
        c_box[i] = display.newImageRect("image/postbox/옷 커스텀창.png", 100, 100)
        c_box[i].alpha = 0
        objectGroup:insert(c_box[i])
    end

    local clothes = {}
    clothes[1] = display.newImageRect("image/postbox/outer4.png", 80, 80) --봄옷
    clothes[1].name = "outer4"
    clothes[2] = display.newImageRect("image/postbox/outer5.png", 80, 80) --여름옷1
    clothes[2].name = "outer5"
    clothes[3] = display.newImageRect("image/postbox/outer6.png", 80, 80) --여름옷2
    clothes[3].name = "outer6"
    clothes[4] = display.newImageRect("image/postbox/outer7.png", 80, 80) --가을옷
    clothes[4].name = "outer7"
    clothes[5] = display.newImageRect("image/postbox/outer8.png", 80, 80) --겨울옷1
    clothes[5].name = "outer8"
    clothes[6] = display.newImageRect("image/postbox/outer9.png", 80, 80) --겨울옷2
    ...
    for i=1, 6 do
        ...
        if(loadedSettings.clothes[i] == true) then --히든 퀘스트 4개 완. 옷을 받았다 = true로 바꿈
            print("요"..i.."!")
            c_box[i].alpha = 1
            c_box[i].x , c_box[i].y = display.contentWidth*0.32+(p_cnt-1)*100, display.contentHeight/2 -140
            clothes[i].alpha = 1
            clothes[i].x, clothes[i].y = c_box[i].x, c_box[i].y
            p_cnt = p_cnt + 1
        end
    end
    --local hQ_num = 8
    ...
    local exit = display.newImageRect("image/설정/닫기.png", 60, 60)
    sceneGroup:insert(exit)
    exit.x, exit.y = display.contentWidth*0.73, display.contentHeight*0.16
