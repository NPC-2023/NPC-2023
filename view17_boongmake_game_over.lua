...
function scene:create( event )
	...
	local background = display.newImageRect("image/boong/view02_background.png",display.contentWidth, display.contentHeight) ---배경
	...
	local status = composer.getVariable("boongmake_status")

	local function backtogame(event) --실패할 경우 다시 게임으로 돌아가기
		if event.phase == "began" then 
				audio.pause(home)
				composer.removeScene("view17_boongmake_game_over")
